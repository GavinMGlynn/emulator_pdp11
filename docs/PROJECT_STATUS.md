...
| RX11/RX01 floppy (programmed I/O) | **working** | `rx11` register probe byte-identical to SimH (power-up init: DONE\|ERR + ecode 010 with no floppy); the RXCS/RXDB transfer-request state machine — fill/empty buffer, read/write sector, read status — round-trips a 128-byte sector through the 8-inch floppy in unit tests |
